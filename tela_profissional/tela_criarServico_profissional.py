...
async def obter_dados_profissional(id_profissional: int) -> Optional[Tuple[str, str]]:
    try:
...
async def criarServico(page: ft.Page):
    ...
    if not id_profissional:
        page.go('/login')
        return
    dados_prof = await obter_dados_profissional(id_profissional)
    nome_completo = f"{dados_prof[0]} {dados_prof[1]}" if dados_prof else "Profissional"
