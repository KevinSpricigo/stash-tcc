...
async def obter_dados_profissional(id_profissional: int) -> Optional[Tuple[str, str, str, str]]:
    try:
        resultado = supabase.table('profissionais')\
            .select('nome, sobrenome, status, profissao')\
        ...
        print(f"Erro ao obter dados do profissional: {e}")
        return None
...
async def home_profissional(page: ft.Page):
    ...
    dados_profissional = await obter_dados_profissional(id_profissional)
    if not dados_profissional:
        page.go('/login')
        return

    nome, sobrenome, status_atual, profissao = dados_profissional
            ...
            controls=[
                ...
                ft.Row(
                    controls=[
                        ...
                        cards_pronto(ft.icons.LOCK, "Indisponível", disabled=True),
                    ],
                    alignment=ft.MainAxisAlignment.CENTER,
                    spacing=20
                ),
